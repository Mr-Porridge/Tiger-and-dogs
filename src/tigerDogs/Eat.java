...
public class Eat {
    ...
    public static void leftRight(int map[][], int[][] id, List<Integer> eaten) {
        ...
        if (option.contains(7) && option.contains(3)) {
            ...
            /*for (int i = y - 1; i < y; i++) {
                for (int j = y; j < y + 2; j++) {
                    if (map[x][i] == 1 && map[x][j] == 1) {
                        map[x][i] = map[x][j] = 0;
                        eaten.add(id[x][i]);
                        eaten.add(id[x][j]);*/

                        /*cb.removeChess(id[x][i]);
                        cb.removeChess(id[x][j]);*/
            /*new magicMusic.Play0("/Users/chenzhixin/Desktop/Tiger_and_Dogs/src/tigerDogs/BGMs/Tiger_eat_dogs.mp3").start();*/
        }
    }
